...
import { colorize, compareVersions, execAsync, formatBytes, formatDuration, pathIndexHas, } from "./utils.js";
/**
...
export async function checkUpdates(options = {}) {
    ...
    const checkOne = async (app) => {
        ...
        if (cmp < 0)
            return { app, update: false, latestVersion, note: `newer than ${latestVersion}` };
    ...
    for (const o of outcomes) {
        if (!options.silent)
            process.stdout.write(`Checking ${o.app.name}... `);
        if (o.update && o.latestVersion) {
            if (!options.silent)
                console.log(colorize(`${o.app.version} → ${o.latestVersion}`, "yellow"));
            updates.push(o.app);
        }
            ...
            if (!options.silent)
                console.log(colorize(`${o.app.version} (${o.note})`, "green"));
        }
        else {
            if (!options.silent)
                console.log(colorize(o.note ?? "unable to check", "gray"));
        }
    }
    if (!options.silent)
        console.log();
 ...
 */
export function updateCommandFor(app) {
    switch (app.source) {
        case "npm":
 ...
 */
export async function runUpdate(app) {
    ...
    if (result.success)
        return { ok: true, command, detail: `${app.name} updated` };
...
export async function monitorProcesses(options = {}) {
    ...
    const runningApps = [];
    let processes = [];
    const ps = await execAsync("powershell -Command \"Get-Process | Select-Object Name, Id, Path, WorkingSet | ConvertTo-Json -Compress\"", [], { shell: true, timeout: 10_000 });
    if (ps.success) {
        try {
            const data = JSON.parse(ps.output);
