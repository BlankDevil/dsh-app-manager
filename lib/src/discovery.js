...
import { appDataDir, dedupeBy, enumPathExecutables, execSafe, getLastSpawnError, homeDir, localAppDataDir, namesLikelyMatch, npmGlobalRoot, npxCacheCandidates, pathIndexHas, pnpmGlobalCandidates, readArpEntries, readPackageJson, } from "./utils.js";
/* -------------------------------------------------------------------------- */
/*  Scan-result cache                                                         */
/* -------------------------------------------------------------------------- */
/**
...
function readBaselineCached() {
    ...
    if (data.length === 0 && process.platform === "win32") {
        ...
        if (why) {
            console.warn(`[dsh-app-manager] ARP baseline unavailable — managed/unmanaged classification ` +
...
function stripLauncherExt(name) {
 ...
 */
function discoverPnpmGlobalRaw() {
    const apps = [];
    if (!pathIndexHas("pnpm"))
        return apps;
    ...
    if (!hasGlobalRoot)
        return apps; // never installed a global package
...
function scanNpxCacheDir(npxCache, apps) {
    try {
 ...
 */
function discoverChocoRaw() {
    const apps = [];
    if (!pathIndexHas("choco"))
        return apps;
    ...
    for (const line of lines) {
        ...
        if (/^Chocolatey v/i.test(line))
            continue;
        if (/packages? installed/i.test(line))
            continue;
 ...
 */
function discoverCargoRaw() {
    const apps = [];
    if (!pathIndexHas("cargo"))
        return apps;
 ...
 */
function discoverPipxRaw() {
    const apps = [];
    if (!pathIndexHas("pipx"))
        return apps;
 ...
 */
function discoverPipGlobalRaw() {
    const apps = [];
    if (!pathIndexHas("pip") && !pathIndexHas("pip3"))
        return apps;
...
function discoverUvToolsRaw() {
    ...
    if (!uvToolDirs.some((d) => existsSync(d)))
        return apps;
